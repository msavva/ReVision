...
%
%
function [best_params, best_inliers, best_error, varargout] = ellipseRANSAC(edgelIm, numIterations, initialInliers, minInliers)
    ...
    while iterations <= numIterations && best_error > 0.8
        ...
        % Fit the ellipse
        ellipseParams = fitellipse(new_inliers(:,1),new_inliers(:,2));
        ellipse_fit_time = ellipse_fit_time + toc(tFit);
        
        % Calculate the error for outlier points. We are using distance from
        % point to ellipse.
        tRes = tic;
        %[err XYproj] = Residuals_ellipse(new_outliers, ellipseParams);
        [err XYproj] = Residuals_ellipse_vector1(new_outliers, ellipseParams);
        clear XYproj
        ...
        if(length(consensus_set) >= minInliers)
            ...
            [dist_error new_proj] = Residuals_ellipse_vector1(consensus_set, new_ellipse);
            ...
            %dist_error = sum(dist_error);
            %dist_error = max(dist_error);
            ...
            good_ellipse_error = 1-min(1,length(unique(proj_indices))/ellipse_circumference(new_ellipse));
...
%             new_proj'
%             size(edgelIm)
%             find(new_proj(:,1) < 1 | new_proj(:,1) > size(edgelIm,2))
%             find(new_proj(:,2) < 1 | new_proj(:,2) > size(edgelIm,1))
%             beep
%             beep
%             pause;
        end
